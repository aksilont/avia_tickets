...
- (void)setup {
    ...
    backgroundImageView.image = [UIImage systemImageNamed:@"cloud"];
...
- (void)createPlanes {
    for (int i = 1; i < 6; i++) {
        ...
        plane.tag = i;
        plane.image = [UIImage systemImageNamed:@"airplane"];
        [self addSubview:plane];
    }
}

- (void)startAnimationg:(NSInteger)planeId {
    if (!isActive) return;
    ...
    if (plane) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.3 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
            [self startAnimationg:(planeId + 1)];
        });
    }
}
